...
export default function SettingsPage() {
  ...
  const exportData = () => {
    try {
      ...
      a.href = url
      a.download = `lubricentro-backup-${new Date().toISOString().split("T")[0]}.json`
    ...
    <DashboardLayout>
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">Ajustes del sistema y gestión de datos locales</p>
        </div>
      </div>
      ...
      <Card className="racing-shadow">
        <CardHeader>
          ...
          <CardDescription>Ajustes básicos de funcionamiento de la aplicación</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>Sincronización Automática</Label>
              <div className="text-sm text-muted-foreground">Sincronizar datos automáticamente cuando hay conexión</div>
            </div>
            <Switch
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <div className="text-sm text-muted-foreground">Mostrar alertas de stock bajo y sincronización</div>
            </div>
            <Switch
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setSettings((prev) => ({ ...prev, lowStockThreshold: Number.parseInt(e.target.value) || 0 }))
              }
              className="w-32"
            />
            <div className="text-sm text-muted-foreground">Cantidad mínima para considerar stock bajo</div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setSettings((prev) => ({ ...prev, markupPercentage: Number.parseFloat(e.target.value) || 0 }))
              }
              className="w-32"
            />
            <div className="text-sm text-muted-foreground">Porcentaje aplicado al costo del producto</div>
          </div>
          ...
          <Button onClick={saveSettings} className="racing-shadow cursor-pointer">
      ...
      <Card className="racing-shadow">
        <CardHeader>
          ...
          <CardDescription>Información sobre almacenamiento local y opciones de respaldo</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Storage Info */}
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div className="text-center p-3 bg-muted/50 rounded-lg">
              <div className="text-2xl font-bold text-primary">{storageInfo.products}</div>
              <div className="text-sm text-muted-foreground">Productos</div>
            </div>
            <div className="text-center p-3 bg-muted/50 rounded-lg">
              <div className="text-2xl font-bold text-accent">{storageInfo.sales}</div>
              <div className="text-sm text-muted-foreground">Ventas Pendientes</div>
            </div>
            <div className="text-center p-3 bg-muted/50 rounded-lg">
              <div className="text-2xl font-bold text-accent">{storageInfo.orders}</div>
              <div className="text-sm text-muted-foreground">Pedidos Pendientes</div>
            </div>
            <div className="text-center p-3 bg-muted/50 rounded-lg">
              <div className="text-2xl font-bold text-primary">{getStorageSize()}</div>
          ...
          {storageInfo.lastBackup && (
            <div className="p-3 bg-muted/50 rounded-lg">
              ...
              <div className="text-sm text-muted-foreground">{storageInfo.lastBackup.toLocaleString()}</div>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button onClick={loadStorageInfo} variant="outline" className="gap-2 bg-transparent">
      ...
      <Card className="racing-shadow">
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <div className="text-sm font-medium">Versión de la Aplicación</div>
              <div className="text-sm text-muted-foreground">1.0.0</div>
            </div>
            <div>
              <div className="text-sm font-medium">Navegador</div>
              <div className="text-sm text-muted-foreground">{navigator.userAgent.split(" ")[0]}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">{navigator.onLine ? "Conectado" : "Sin conexión"}</div>
            </div>
          </div>
        </CardContent>
      </Card>
