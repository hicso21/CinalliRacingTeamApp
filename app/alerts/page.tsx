...
import { AlertTriangle, Wifi, WifiOff, RefreshCw, Package, ShoppingCart, TrendingUp, CheckCircle } from "lucide-react"
    ...
    try {
      ...
      if (OfflineSync.isOnline()) {
        ...
        if (productsData) {
          ...
          const lowStock = productsData.filter((product) => product.stock <= product.min_stock)
          setLowStockProducts(lowStock)
        }
      } else {
        const localProducts = OfflineSync.getProductsFromLocal()
        setProducts(localProducts)
        const lowStock = localProducts.filter((product) => product.stock <= product.min_stock)
        setLowStockProducts(lowStock)
      }
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-primary">Alertas y Sincronización</h2>
      ...
      <Card className="racing-shadow">
        <CardHeader>
          ...
          <CardDescription>Información sobre la conectividad y sincronización de datos</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${isOnline ? "bg-green-500" : "bg-destructive"}`}></div>
              <div>
                <div className="font-medium">{isOnline ? "Conectado" : "Sin conexión"}</div>
                <div className="text-sm text-muted-foreground">
                  {isOnline ? "Datos sincronizados en tiempo real" : "Trabajando en modo offline"}
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${totalPendingItems === 0 ? "bg-green-500" : "bg-accent"}`}></div>
              <div>
                <div className="font-medium">{totalPendingItems} elementos pendientes</div>
                <div className="text-sm text-muted-foreground">
                  {totalPendingItems === 0 ? "Todo sincronizado" : "Esperando sincronización"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{lastSync ? lastSync.toLocaleString() : "Nunca"}</div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      {totalPendingItems > 0 && (
        <Card className="racing-shadow border-accent">
          <CardHeader>
            ...
            <CardDescription>Datos guardados localmente que se sincronizarán cuando haya conexión</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {pendingSales.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium">Ventas Pendientes ({pendingSales.length})</span>
                  </div>
                    ...
                    {pendingSales.slice(0, 5).map((sale) => (
                      <div key={sale.id} className="flex items-center justify-between p-2 bg-muted/50 rounded">
                        <div className="text-sm">
                          {getProductName(sale.product_id)} - {sale.quantity} unidades
                        </div>
                        <div className="text-sm font-medium">${sale.total.toLocaleString()}</div>
                      </div>
                    ))}
                    {pendingSales.length > 5 && (
                      <div className="text-sm text-muted-foreground">Y {pendingSales.length - 5} ventas más...</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {pendingOrders.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium">Pedidos Pendientes ({pendingOrders.length})</span>
                  </div>
                  <div className="space-y-2">
                    {pendingOrders.slice(0, 5).map((order) => (
                      <div key={order.id} className="flex items-center justify-between p-2 bg-muted/50 rounded">
                        <div className="text-sm">
                          {getProductName(order.product_id)} - {order.quantity} unidades
                        </div>
                        <div className="text-sm font-medium">${order.total.toLocaleString()}</div>
                      </div>
                    ))}
                    {pendingOrders.length > 5 && (
                      <div className="text-sm text-muted-foreground">Y {pendingOrders.length - 5} pedidos más...</div>
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-3">
              {lowStockProducts.map((product) => (
                <div key={product.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${product.stock === 0 ? "bg-destructive" : "bg-accent"}`}
                    ></div>
                    <div>
      ...
      <Card className="racing-shadow">
        <CardHeader>
          ...
          <CardDescription>Información general sobre el estado de la aplicación</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-accent">{totalPendingItems}</div>
              <div className="text-sm text-muted-foreground">Elementos pendientes</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-500">{isOnline ? "100%" : "0%"}</div>
