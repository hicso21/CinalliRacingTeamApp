...
export class OfflineSync {
  ...
  private static getStorage() {
    if (!this.storage) {
      throw new Error('OfflineSync: Storage no inicializado. Llama a OfflineSync.setStorage() primero.')
    }
    return this.storage
  }
  ...
  static saveProductsToLocal(products: Product[]): void {
    try {
      ...
      storage.setItem(STORAGE_KEYS.PRODUCTS_TIMESTAMP, new Date().toISOString())
  ...
  static markLastSync(): void {
    try {
      const storage = this.getStorage()
      storage.setItem(STORAGE_KEYS.LAST_SYNC_TIMESTAMP, new Date().toISOString())
    } catch (error) {
  ...
  static getSyncStats() {
    try {
      ...
      return {
        ...
        localProductsTimestamp: storage.getItem(STORAGE_KEYS.PRODUCTS_TIMESTAMP),
      }
    } catch (error) {
  ...
  static validateLocalData(): {
    ...
    try {
      ...
      const productsTimestamp = storage.getItem(STORAGE_KEYS.PRODUCTS_TIMESTAMP)
  ...
  static importLocalData(data: {
    ...
    try {
      ...
      if (data.pendingOrders) {
        storage.setItem(STORAGE_KEYS.PURCHASE_ORDERS, JSON.stringify(data.pendingOrders))
      }

      return {
        success: true,
        message: `Datos importados: ${data.products?.length || 0} productos, ${data.pendingSales?.length || 0} ventas, ${data.pendingOrders?.length || 0} pedidos`
      }
    } catch (error: any) {
      return {
        success: false,
